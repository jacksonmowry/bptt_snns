...
inline void sleep(const double t) {
    if (t > 0.0) {
        std::this_thread::sleep_for(
            std::chrono::milliseconds((int)(1E3 * t + 0.5)));
    }
}

template <class T2, class T1> T2 cpp11_bit_cast(T1 t1) {
...
inline float as_float(const uint x) { return cpp11_bit_cast<float>(x); }
inline uint as_uint(const float x) { return cpp11_bit_cast<uint>(x); }
inline double as_double(const ulong x) { return cpp11_bit_cast<double>(x); }
inline ulong as_ulong(const double x) { return cpp11_bit_cast<ulong>(x); }
...
inline float cb(const float x) { return x * x * x; }
...
inline float sign(const float x) { return x >= 0.0f ? 1.0f : -1.0f; }
inline float clamp(const float x, const float a, const float b) {
    return fmin(fmax(x, a), b);
}
inline float rsqrt(const float x) { return 1.0f / sqrt(x); }
...
inline double cb(const double x) { return x * x * x; }
...
inline int sign(const int x) { return 1 - 2 * (x >> 31 & 1); }
...
inline uint sq(const uint x) { return x * x; }
inline uint cb(const uint x) { return x * x * x; }
...
inline slong sq(const slong x) { return x * x; }
inline slong cb(const slong x) { return x * x * x; }
...
inline slong sign(const slong x) { return 1ll - 2ll * (x >> 63 & 1ll); }
inline slong min(const slong x, const slong y) { return x < y ? x : y; }
inline slong max(const slong x, const slong y) { return x > y ? x : y; }
inline slong clamp(const slong x, const slong a, const slong b) {
    return min(max(x, a), b);
}

inline ulong sq(const ulong x) { return x * x; }
inline ulong cb(const ulong x) { return x * x * x; }
...
inline ulong min(const ulong x, const ulong y) { return x < y ? x : y; }
inline ulong max(const ulong x, const ulong y) { return x > y ? x : y; }
...
inline int to_int(const float x) { return (int)(x + 0.5f - (float)(x < 0.0f)); }
inline int to_int(const double x) { return (int)(x + 0.5 - (double)(x < 0.0)); }
inline uint to_uint(const float x) { return (uint)fmax(x + 0.5f, 0.5f); }
inline uint to_uint(const double x) { return (uint)fmax(x + 0.5, 0.5); }
inline slong to_slong(const float x) { return (slong)(x + 0.5f); }
inline slong to_slong(const double x) { return (slong)(x + 0.5); }
inline ulong to_ulong(const float x) { return (ulong)fmax(x + 0.5f, 0.5f); }
inline ulong to_ulong(const double x) { return (ulong)fmax(x + 0.5, 0.5); }
...
inline void split_float(float x, uint& integral, uint& decimal, int& exponent) {
    ...
    if (remainder - (float)decimal >=
        0.5f) { // correct rounding of last decimal digit
        decimal++;
...
inline void split_double(double x, uint& integral, ulong& decimal,
                         int& exponent) {
    ...
    if (remainder - (double)decimal >=
        0.5) { // correct rounding of last decimal digit
        decimal++;
...
inline string to_string(uchar c) { return string(1, c); }
...
inline string to_string_hex(slong x) { return to_string_hex(*(ulong*)&x); }
...
inline string to_string_hex(int x) { return to_string_hex(*(uint*)&x); }
inline string
to_string(float x) { // convert float to string with full precision (<string>
                     // to_string() prints only 6 decimals)
    string s = "";
    if (x < 0.0f) {
        s += "-";
        x = -x;
    ...
    return s + to_string(integral) + "." + decimal_to_string_float(decimal, 8) +
           (exponent != 0 ? "E" + to_string(exponent) : "");
}
inline string
to_string(double x) { // convert double to string with full precision (<string>
                      // to_string() prints only 6 decimals)
    string s = "";
    if (x < 0.0) {
        s += "-";
    ...
    return s + to_string(integral) + "." +
           decimal_to_string_double(decimal, 16) +
           (exponent != 0 ? "E" + to_string(exponent) : "");
}
inline string to_string(float x,
                        const uint decimals) { // convert float to string with
...
inline string to_string(double x,
    ...
    return s + to_string(integral) +
           (decimals == 0u ? ""
                           : "." + decimal_to_string_double(
                                       decimal, min((int)decimals, 16)));
}

inline uint length(const string& s) { return (uint)s.length(); }
...
inline string replace(const string& s, const string& from, const string& to) {
    string r = s;
    int p    = 0;
    while ((p = (int)r.find(from, p)) != (int)string::npos) {
        r.replace(p, from.length(), to);
        p += (int)to.length();
    }
    return r;
}
inline string substring(const string& s, const uint start,
                        uint length = max_uint) {
    return s.substr(start, min(length, (uint)s.length() - start));
}
inline string trim(const string& s) { // removes whitespace characters from
                                      // beginnig and end of string s
...
inline string
alignl(const uint n,
       const string& x = "") { // converts x to string with spaces behind such
                               // that length is n if x is not longer than n
    string s = x;
    for (uint i = 0u; i < n; i++) {
        s += " ";
    }
...
inline string
alignr(const uint n,
       const string& x = "") { // converts x to string with spaces in front such
                               // that length is n if x is not longer than n
    string s = "";
    for (uint i = 0u; i < n; i++) {
        s += " ";
...
inline string
alignl(const uint n,
       const T x) { // converts x to string with spaces behind such that length
                    // is n if x does not have more digits than n
    return alignl(n, to_string(x));
}
template <typename T>
inline string
alignr(const uint n,
       const T x) { // converts x to string with spaces in front such that
                    // length is n if x does not have more digits than n
    return alignr(n, to_string(x));
}

inline void print(const string& s = "") { std::cout << s; }
inline void println(const string& s = "") { std::cout << s + "\n"; }
inline void reprint(const string& s = "") { std::cout << "\r" + s; }
inline void wait() { std::cin.get(); }
template <typename T> inline void println(const T& x) { println(to_string(x)); }

#ifdef UTILITIES_REGEX
inline vector<string> split_regex(const string& s,
...
inline bool equals_regex(
    const string& s,
    const string& match) { // returns true if string exactly matches regex
    return regex_match(s.begin(), s.end(), std::regex(match));
}
inline uint matches_regex(const string& s,
                          const string& match) { // counts number of matches
...
inline string replace_regex(const string& s, const string& from,
                            const string& to) {
    return regex_replace(s, std::regex(from), to);
}
inline bool is_number(const string& s) {
    return equals_regex(s, "\\d+(u|l|ul|ll|ull)?") ||
           equals_regex(s, "0x(\\d|[a-fA-F])+(u|l|ul|ll|ull)?") ||
           equals_regex(s, "0b[01]+(u|l|ul|ll|ull)?") ||
           equals_regex(s, "(((\\d+\\.?\\d*|\\.\\d+)([eE][+-]?\\d+[fF]?)?)|("
                           "\\d+\\.\\d*|\\.\\d+)[fF]?)");
}
inline void print_message(const string& message, const string& keyword = "",
                          const int colons = true) { // print formatted message
    ...
    for (uint i = 0u; i < (uint)v.size(); i++) {
        ...
        if (l <= w + 1u) {    // word fits -> append word and space
            p += word + " ";
                   ...
                   w) { // word overflows -> split word into next line
            p += substring(word, 0, w - (l - wordlength - 1u)) + " |\n| " + f;
            v[i] = substring(v[i], w - (l - wordlength - 1u));
            i--; // reuse same vector element for overflowing part, decrement i
                 // to start next line with this overflowing part
            l = 0u; // reset line length
        } else {    // word does not fit -> fill remaining line with spaces
            l = l - length(v.at(i--)) -
                1u; // remove word from line, decrement i to start next line
                    // with this word
            for (uint j = l; j <= w; j++) {
                p += " ";
            }
...
inline void parse_sanity_check_error(const string& s, const string& regex,
                                     const string& type) {
...
inline void print_message(const string& message, const string& keyword = "",
                          const int colons = true) { // print message
...
inline void set_environment_variable(
    char* s) { // usage: set_environment_variable((char*)"VARIABLE=VALUE");
#if defined(_WIN32)
    (void)_putenv(s);
...
inline vector<string> find_files(const string& path,
...
inline void
create_folder(const string& path) { // create folder if it not already exists
    const int slash_position = (int)path.rfind(
        '/'); // find last slash dividing the path from the filename
    if (slash_position == (int)string::npos) {
        return; // no slash found
    }
    const string f =
        path.substr(0, slash_position); // cut off file name if there is any
#ifndef UTILITIES_NO_CPP17
    if (!std::filesystem::is_directory(f) || !std::filesystem::exists(f)) {
        std::filesystem::create_directories(
            f); // create folder if it not already exists
    }
#endif // UTILITIES_NO_CPP17
}
inline string create_file_extension(const string& filename,
                                    const string& extension) {
    return filename.substr(0, filename.rfind('.')) +
           (extension.at(0) != '.' ? "." : "") +
           extension; // remove existing file extension if existing and replace
                      // it with new one
}
