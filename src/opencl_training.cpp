...
static void read_gpu_weights(const Memory<short>& gpu_weights,
...
static void write_weights_to_network(neuro::Network* n,
                                      size_t total_neurons,
                                      bool discrete,
                                      double scale_factor,
...
bool opencl_train(const CliConfig& cfg,
                  neuro::Network* n,
                  const NetworkConfiguration& nc,
                  const Dataset& train,
                  const Dataset& test,
    ...
    for (size_t epoch = 0; epoch < epochs; epoch++) {
        ...
        printf("Epoch: %4zu/%zu, Loss: %10g / %10g, Acc: %10g / %10g, Test Loss: %10g / %10g, Test Acc: %10g / %10g\n",
               epoch + 1, epochs, avg_train_loss, best_loss, avg_train_acc, best_acc, avg_test_loss, best_test_loss, avg_test_acc, best_test_acc);
    ...
    weights.read_from_device();
    read_gpu_weights(weights, nc.total_neurons, max_incoming,
                     state->weights, nc.scale_factor, state->weights);

    /* CPU final test eval */
    write_weights_to_network(n, nc.total_neurons, nc.discrete,
                             nc.scale_factor, state->weights);
    std::pair<double,double> final_result = cpu_eval_test(n, nc, test,
        state->weights, state->delays, state->thresholds, rho, tau);
