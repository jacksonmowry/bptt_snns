...
ThreadArgs::ThreadArgs(size_t total_neurons, size_t timesteps, size_t output_neurons,
    ...
    : tb(total_neurons, timesteps, output_neurons, rho, tau, weights, delays, thresholds),
      nc(nc), order(order), train(train), test(test),
      max_idx(max_idx), work_idx(work_idx), done(done),
      mut(mut), have_work(have_work), done_work(done_work),
      train_p(train_p), die(die) {}
