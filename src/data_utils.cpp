...
size_t label_count(const Dataset* d) { return (size_t)d->labels.label_strings_count; }
...
void encode_spikes(neuro::Processor* p, const Dataset* d, size_t index,
    ...
    if (timeseries) {
        ...
        for (size_t input = 0; input < input_neurons / 2; input++) {
            double range = d->data.max_vals[input] - d->data.min_vals[input];
        ...
        for (size_t input = 0; input < input_neurons / 2; input++) {
            double x =
                (d->data.data[index * d->data.shape[1] + input] - d->data.min_vals[input]) /
                (d->data.max_vals[input] - d->data.min_vals[input]);
            double inv_x = 1.0 - x;
            if (x > 0.0) {
