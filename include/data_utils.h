 ...
 * values: array of n_features values (already normalized to [0,1] OR raw with
 * min/max). If use_normalized is true, values are assumed [0,1] (no min/max
 * lookup). Returns [num_neurons][timesteps] boolean matrix where num_neurons =
 * n_features * 2.
 */
